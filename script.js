...
function init() {
    ...
    setupAutocomplete(inputs.modern, document.getElementById('autocompleteModern'));
    setupAutocomplete(inputs.retro, document.getElementById('autocompleteRetro'));
}
...
function setupAutocomplete(input, list) {
    ...
    input.addEventListener('input', function () {
        ...
        if (!val || !window.POKEMON_DB) return;

        list.innerHTML = '';
        list.classList.remove('hidden');

        // Filter: Name starts with val OR contains val (priority to starts with?)
        // Let's do a simple includes for now, or statsWith for better "search" feel
        // User asked: "desplegable automático que se genere cuando coloces la primera letra"

        const matches = window.POKEMON_DB.filter(p => p.name.startsWith(val)).slice(0, 5); // Limit to top 5
...
function setBtnLoading(loading) {
    if (loading) closeAllLists(); // Close autocomplete when checking
